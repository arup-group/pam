...
def test_get_legs_data(population):
    ...
    assert len(legs) == 18
    assert list(legs["mode"]) == ['car', 'car', 'car', 'car', 'bike', 'bike', 'walk', 'bus', 'walk', 'walk', 'bus', 'walk', 'walk', 'pt', 'walk', 'walk', 'pt', 'walk']
...
def test_benchmark_trips_hour(population):
    ...
    expected_benchmark = pd.DataFrame(
        {
            'departure_hour': {0: 7, 1: 8, 2: 16, 3: 17},
            'freq_sum': {0: 2, 1: 3, 2: 2, 3: 3}
            }
        )
...
def test_mode_counts_benchmark(population):
    ...
    expected_benchmark = pd.DataFrame(
        {
            'mode': {0: "bike", 1: "bus", 2: "car", 3: "pt"},
            'trips': {0: 2, 1: 2, 2: 4, 3: 2}
            }
        )
    assert bm.equals(expected_benchmark)
