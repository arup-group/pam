...
def person_without_a_vehicle():
    return Person(
        "Bobby", attributes={"age": 6, "job": "education", "gender": "non-binary"}
    )
...
def test_generates_matsim_vehicles_xml_file_containing_expected_vehicles(
    ...
    assert expected_vehicles == {
        vehicle.get("id"): vehicle.get("type") for vehicle in vehicles
    }
...
def test_writing_electric_vehicles_results_in_valid_xml_file(
    ...
    write_electric_vehicles(
        tmpdir, population_with_electric_vehicles.electric_vehicles()
    )
...
def ev_population_xml_path():
    return os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "test_data", "vehicles", "ev_population.xml"
        )
    )
...
def all_vehicle_xml_path():
    return os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "test_data", "vehicles", "all_vehicles.xml"
        )
    )
...
def electric_vehicles_xml_path():
    return os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "test_data", "vehicles", "electric_vehicles.xml"
        )
    )
...
def test_reading_all_vehicles_file(
    all_vehicle_xml_path, expected_all_vehicle_xml_output
):
...
def test_reading_population_with_both_vehicle_files_assigns_all_vehicles_correctly(
    ...
    for person in ["Eddy", "Stevie", "Vladya"]:
        pop.get(person).people[person].vehicle = expected_electric_vehicle_xml_output[
            person
        ]
