...
def test_too_long_plan_to_array():
    ...
    np.testing.assert_array_equal(
        encode.plan_to_one_hot(plan=plan, mapping={"home": 0, "work": 1, "travel": 2}, bin_size=3600, duration=86401),
...
def test_iter_array():
    assert list(decode.iter_array(array=np.stack([np.array((1, 0, 0))] * 3), mapping={0: "home"})) == [
        ("home", datetime(year=1900, month=1, day=1, hour=0, minute=0, second=0))
    ]

    assert list(decode.iter_array(array=np.stack([np.array((0, 1, 0))] * 3), mapping={0: "home", 1: "work"})) == [
...
def test_add_end_times():
    start_times = [datetime(year=1900, month=1, day=1, hour=i, minute=0, second=0) for i in range(3)]
...
def test_plan_one_hot_encode_decode_consistent():
    ...
    decoded_plan = decode.one_hot_to_plan(
        array=encoded_plan, mapping={0: "home", 1: "work", 2: "travel"}, bin_size=3600, duration=86400
    )
