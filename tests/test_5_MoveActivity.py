...
def test_tour_matches_activities_returns_True_when_there_is_a_matching_tour_and_plain_filter(activity_tours, plain_filter):
    assert MoveActivityTourToHomeLocation(['education', 'shop']).tour_matches_activities(activity_tours[0], plain_filter)
    ...
    def is_a_selected_activity(act):
        for other_act in activity_tours[0]:
            if act.is_exact(other_act):
                return True
        return False
    assert MoveActivityTourToHomeLocation(['education', 'shop']).tour_matches_activities(activity_tours[0], is_a_selected_activity)
...
def test_tour_matches_activities_returns_False_when_there_is_no_matching_tour_and_plain_filter(activity_tours, plain_filter):
    assert not MoveActivityTourToHomeLocation(['education', 'shop']).tour_matches_activities(activity_tours[1], plain_filter)
