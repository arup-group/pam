...
def test_apply_bin_integer_transformer_to_michael(michael, bins):
    assert attributes.bin_integer_transformer(
        michael, 'age', bins) == 'younger'
...
def test_apply_discrete_joint_distribution_sampler_to_michael(michael, cat_joint_distribution):
    ...
    assert attributes.discrete_joint_distribution_sampler(
        michael, mapping, dist) == False


def test_applt_discrete_joint_distribution_sampler_to_kasia(kasia, cat_joint_distribution):
    mapping, dist = cat_joint_distribution
    assert attributes.discrete_joint_distribution_sampler(
        kasia, mapping, dist) == True
...
def test_applt_discrete_joint_distribution_sampler_reproducibility_to_michael_carefully(michael, dog_joint_distribution, fixed_seed):
    ...
    assert attributes.discrete_joint_distribution_sampler(
        michael, mapping, dist, careful=True, seed=fixed_seed) == True
...
def test_applt_discrete_joint_distribution_sampler_reproducibility_to_kasia_carefully(kasia, dog_joint_distribution, fixed_seed2):
    mapping, dist = dog_joint_distribution
    assert attributes.discrete_joint_distribution_sampler(
        kasia, mapping, dist, careful=True, seed=fixed_seed2) == False
...
def test_sample_point_from_geoseries_of_polygons():
    ...
    assert isinstance(
        sampler.sample_point_from_polygon(gdf.geometry[0]), Point)
...
def test_random_sample_point_from_multipolygon():
    ...
    sampler = spatial.RandomPointSampler(gdf.geometry, patience=0)
    assert isinstance(
        sampler.sample_point_from_multipolygon(gdf.geometry[0]), Point)
...
def test_random_sample_point_from_multipolygon_random_seed(fixed_seed):
    ...
    poly = MultiPolygon([p1, p2])
    gdf = gp.GeoDataFrame(df, geometry=[poly]*3)
    sampler = spatial.RandomPointSampler(
        gdf.geometry, patience=0, seed=fixed_seed)
    assert list(sampler.sample_point_from_multipolygon(
        gdf.geometry[0]).coords[0]) == [0.13436424411240122, 0.8474337369372327]
...
def test_random_sample_point_from_multilinestring():
    ...
    sampler = spatial.RandomPointSampler(gdf.geometry, patience=0)
    assert isinstance(
        sampler.sample_point_from_multilinestring(gdf.geometry[0]), Point)
...
def test_random_sample_point_from_multilinestring_random_seed(fixed_seed):
    ...
    sampler = spatial.RandomPointSampler(
        gdf.geometry, patience=0, seed=fixed_seed)
    assert list(sampler.sample_point_from_multipolygon(
        gdf.geometry[0]).coords[0]) == [10.134364244112401, 10.847433736937234]
...
def test_random_sample_point_from_multipoint():
    ...
    sampler = spatial.RandomPointSampler(gdf.geometry, patience=0)
    assert isinstance(
        sampler.sample_point_from_multipoint(gdf.geometry[0]), Point)


def test_sample_geom_from_series():
    df = pd.DataFrame({1: [0, 0, 3], 2: [4, 5, 6]})
    poly = Polygon(((0, 0), (1, 0), (1, 1), (0, 1)))
    gdf = gp.GeoDataFrame(df, geometry=[poly]*3)
    gdf = gdf.set_index(1)
    sampler = spatial.RandomPointSampler(gdf, patience=0)
    assert isinstance(
        sampler.sample_geom_from_series(0), Polygon)


def test_sample_geom_from_series_mixed_geom_types():
    df = pd.DataFrame({1: [0, 0, 3], 2: [4, 5, 6]})
    p1 = Point((0, 0))
    poly = Polygon(((0, 0), (1, 0), (1, 1), (0, 1)))
    gdf = gp.GeoDataFrame(df, geometry=[p1, poly, poly])
    gdf = gdf.set_index(1)
    sampler = spatial.RandomPointSampler(gdf, patience=0)
    assert isinstance(
        sampler.sample_geom_from_series(0), Polygon)
...
def test_random_sample_point_from_multipoint_random_seed(fixed_seed):
    ...
    sampler = spatial.RandomPointSampler(
        gdf.geometry, patience=0, seed=fixed_seed)
    assert list(sampler.sample_point_from_multipolygon(
        gdf.geometry[0]).coords[0]) == [10.0, 10.0]
...
def test_random_point_from_geoseries_of_polygons_with_shared_index():
    df = pd.DataFrame({1: [0, 0, 3], 2: [4, 5, 6]})
    poly = Polygon(((0, 0), (1, 0), (1, 1), (0, 1)))
    gdf = gp.GeoDataFrame(df, geometry=[poly]*3)
    gdf = gdf.set_index(1)
...
def test_random_point_from_geoseries_of_polygons_random_seed(fixed_seed):
    ...
    gdf = gp.GeoDataFrame(df, geometry=[poly]*3)
    sampler = spatial.RandomPointSampler(gdf.geometry, seed=fixed_seed)
    assert sampler.sample(0, None).coords[0] == (
        0.13436424411240122, 0.8474337369372327)
...
def test_random_point_from_geodataframe_of_polygons_random_seed(fixed_seed):
    ...
    assert sampler.sample(0, None).coords[0] == (
        0.13436424411240122, 0.8474337369372327)
    assert sampler.sample(1, None).coords[0] == (
        10.134364244112401, 10.847433736937234)
    assert sampler.sample(2, None).coords[0] == (
        0.13436424411240122, 0.8474337369372327)
...
def test_inf_yield_weighted_random_seed(fixed_seed):
    ...
    sampler = facility.inf_yielder(
        candidates, weights=weights, seed=fixed_seed)
    assert [next(sampler(None, None, None)) for i in range(3)] == [3, 3, 3]


def test_facility_dict_build():

    facility_df = pd.DataFrame({'id': [1, 2, 3, 4], 'activity': [
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'])
...
def test_facility_sampler_missing_activity_random_sample_fixed_seed(fixed_seed):
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], seed=fixed_seed)
    assert sampler.sample(0, 'education').coords[0] == (
        0.26872848822480244, 1.6948674738744653)
...
def test_facility_sampler_missing_activity_return_None():
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], random_default=False, fail=False)
...
def test_facility_sampler_missing_activity_fail():
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], random_default=False, fail=test_applt_discrete_joint_distribution_sampler_to_fred_not_carefully)
...
def test_facility_sampler_weighted():
    ...
    facility_df = pd.DataFrame({'id': [1, 2, 3, 4], 'activity': [
                               'home', 'home', 'home', 'education'], 'floorspace': [0, 200, 700, 100]})
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], weight_on='floorspace')
...
def test_facility_sampler_weighted_maxwalk():
    ...
    points = [Point((1, 1)), Point((1.5, 1.5)),
              Point((1.8, 1.8)), Point((3, 3))]
    facility_gdf = gp.GeoDataFrame(facility_df, geometry=points)
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], weight_on='floorspace', max_walk=1000)
...
def test_facility_sampler_missing_activity_return_None_weighted():
    ...
    facility_df = pd.DataFrame({'id': [1, 2, 3, 4], 'activity': [
                               'home', 'work', 'home', 'education'], 'floorspace': [0, 200, 700, 100]})
    points = [Point((1, 1)), Point((1, 1)), Point((3, 3)), Point((3, 3))]
    facility_gdf = gp.GeoDataFrame(facility_df, geometry=points)
    ...
    sampler = facility.FacilitySampler(facility_gdf, zones_gdf, [
                                       'home', 'work', 'education'], random_default=False, fail=False, weight_on='floorspace')
    assert sampler.sample(0, 'education') is None
...
def test_facility_sampler_weighted_distance():
    ...
    facility_df = pd.DataFrame({'id': [1, 2, 3, 4], 'activity': [
                               'work', 'work', 'work', 'education'], 'floorspace': [1, 1, 10, 10]})
    points = [Point((0.01, 0.01)), Point((1000, 750)),
              Point((100, 100)), Point((30000, 30000))]
    facility_gdf = gp.GeoDataFrame(facility_df, geometry=points)

    zones_df = pd.DataFrame({'a': [1, 2, 3], 'b': [4, 5, 6]})
    polys = [
        Polygon(((0, 0), (0, 2000), (2000, 2000), (2000, 0))),
        Polygon(((20000, 20000), (20000, 40000),
                (40000, 40000), (40000, 20000))),
    ...
    sampled_facilities = []
    for i in range(20):
        sampled_facilities.append(sampler.sample(
            0, 'work', mode='walk', previous_duration=pd.Timedelta(minutes=15), previous_loc=Point(0, 0)))
    assert pd.Series(sampled_facilities).value_counts(
        normalize=True).idxmax() == Point((1000, 750))
