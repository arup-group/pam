#!/usr/bin/env bash
###########################################################
# Script to profile PAM's matsim reader for both
...
function usage() {
    echo ""
    echo "usage: $script_name [-pbmrh]"
    echo "  -p    The full path to the MATSim plans XML file to load (mandatory)"
...
while getopts ":p:b:m:r:h" opt; do
  case $opt in
    p) plans="$OPTARG"
    ;;
...
pushd "${0%/*}"
printf "\nProfiling PAM MATSim reader....\n"

printf "Running PAM read read_matsim...\n"
printf "\n${QUOTE}-----------------------------------------------\n"
export PYTHONPATH=../..
env | grep -i pythonpath
mprof run --python  --include-children --exit-code python matsim_reader.py -p $plans
return_code=$?
echo "-----------------------------------------------"
printf "\n${PLAIN}"
...
then
  printf "PAM MATSim reader exited normally\n"
else
  printf "${FAILURE}${CROSS_SYMBOL}  !!! PAM MATSim reader exited with an error (exit code=${return_code})!!! \
${CROSS_SYMBOL}\n${PLAIN}"
  exit $return_code
fi
...
mprof plot $profile_file -o $plot_file -t "PAM MATSim Reader Profile ($profile_file)"
